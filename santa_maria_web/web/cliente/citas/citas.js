...
let citas = [];
let d = document;
let usuario = localStorage.getItem('token');//para filtrar las citas de este cliente
...
export function refrescarTabla() {
    let url = "../servicio/cita/search?datoBusqueda="+usuario;
    fetch(url)
            .then(response => {
...
export function verActivos() {
    let url = "../servicio/cita/searchActives?datoBusqueda="+usuario;
    fetch(url)
            .then(response => {
                return response.json();
            })
...
export function verInactivos() {
    let url = "../servicio/cita/searchInactives?datoBusqueda="+usuario;
    fetch(url)
            .then(response => {
                return response.json();
            })
...
function fillTable(data) {
    ...
    for (let i = 0; i < data.length; i++)
    {
        ...
        contenido += '<tr>' +
                ...
                '<td>Empleado: ' + data[i].empleado.persona.nombre + ' ' + data[i].empleado.persona.apellidoP + ' ' + data[i].empleado.persona.apellidoM + '</td>' +
                '<td class="' + (data[i].estatus === 1 ? 'Activo' : 'Inactivo') + '">' + (data[i].estatus === 1 ? 'Agendada' : 'Cancelada o Concluida') + '</td>' +
                '<td class="text-info">' +
                '<a title="Cargar Detalles" href="#" onclick="cit_cli.cargarDetalles(' + i +
