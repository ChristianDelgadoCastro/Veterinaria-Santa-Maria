...
function fillTable(data) {
    ...
    for (let i = 0; i < data.length; i++)
    {
        ...
        contenido += '<tr>' +
                ...
                '<td>Empleado: ' + data[i].cliente.persona.nombre + ' ' + data[i].cliente.persona.apellidoP + ' ' + data[i].cliente.persona.apellidoM + '</td>' +
                '<td class="' + (data[i].estatus === 1 ? 'Activo' : 'Inactivo') + '">' + (data[i].estatus === 1 ? 'Agendada' : 'Cancelada o Concluida') + '</td>' +
                '<td class="text-info">' +
                '<a title="Cargar Detalles" href="#" onclick="cit_vet.cargarDetalles(' + i +
